...
const EditMedicationPage = () => {
  ...
  const fetchMedications = async (pageOverride?: number) => {
    try {
      setLoading(true);
      ...
      if (trimmed) {
        query = query.or(
          `name.ilike.%${trimmed}%,combination.ilike.%${trimmed}%,generic_name.ilike.%${trimmed}%`
        );
      }
      ...
      setTotalMedications(count || 0);
      setMedications((data || []).map((m: any) => ({ ...m, batchCount: m.batchCount ?? 0 })));
    } catch (error) {
  ...
  const fetchBatches = async (medicationId: string) => {
    try {
      ...
      setMedications(prev => prev.map(med =>
        med.id === medicationId
          ? { ...med, batches: data || [], batchCount: (data || []).length }
          : med
      ));
    } catch (error) {
      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        <div className="mb-4 bg-white rounded-xl border border-gray-100 shadow-sm p-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="text-sm text-gray-600">
            Showing {(totalMedications === 0 ? 0 : (currentPage - 1) * pageSize + 1)}-
            {Math.min(currentPage * pageSize, totalMedications)} of {totalMedications}
          </div>
          <div className="flex items-center gap-3">
            <select
              value={pageSize}
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="text-right">
                        ...
                        <div className="text-lg font-bold text-purple-600">
                          {medication.batchCount ?? (medication.batches ? medication.batches.length : 0)}
                        </div>
                      </div>
