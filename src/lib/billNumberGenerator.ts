 ...
 * 
 * Includes retry logic to handle race conditions and duplicate bill numbers.
 * 
 ...
 * @param maxRetries Maximum number of retries if duplicate is detected (default: 3)
 */
export async function generateSequentialBillNumber(
  ...
  while (attempt < maxRetries) {
    try {
      ...
      // On retry attempts, add the attempt number to avoid collisions
      if (attempt > 0) {
        sequence += attempt;
      }
      ...
      const billNumber = `${fullPrefix}-${sequenceStr}`;
      
      // Verify this bill number doesn't already exist (extra safety check)
      const { data: existingCheck, error: checkError } = await supabase
        .from(tableName)
        .select(columnName)
        .eq(columnName, billNumber)
        .limit(1);
        
      if (checkError) {
        console.warn('Error checking for existing bill number:', checkError);
      }
      
      // If no duplicate found, return the bill number
      if (!existingCheck || existingCheck.length === 0) {
        return billNumber;
      }
      
      // Duplicate found, increment attempt and try again
      console.warn(`Duplicate bill number detected: ${billNumber}, retrying attempt ${attempt + 1}`);
      attempt++;
  ...
  // If all retries failed, generate a unique bill number with timestamp and random suffix
  const timestamp = Date.now().toString().slice(-4);
  const randomSuffix = Math.floor(Math.random() * 100).toString().padStart(2, '0');
  const fallbackBillNumber = `${fullPrefix}-${timestamp}${randomSuffix}`;
  
  console.warn(`Max retries reached, using fallback bill number: ${fallbackBillNumber}`);
  return fallbackBillNumber;
}
