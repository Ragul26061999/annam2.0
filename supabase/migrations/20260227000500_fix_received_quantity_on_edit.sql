-- Make medicine_batches.received_quantity adjust by delta on edits (replace semantics)
-- Previously it only increased on positive deltas, causing received to accumulate after edits.
...
BEGIN
  ...
  IF v_delta_units <> 0 THEN
    ...
    IF EXISTS (
      ...
      BEGIN
        -- Replace semantics: received_quantity tracks the net received after edits (same delta as current_quantity)
