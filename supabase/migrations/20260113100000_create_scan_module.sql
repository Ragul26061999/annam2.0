...
-- 1. SCAN TEST CATALOG
-- Stores available scan test definitions (CT, MRI, Ultrasound, etc.)
...
-- 2. SCAN TEST ORDERS
-- Stores scan test orders from doctors (CT, MRI, Ultrasound, etc.)
...
CREATE POLICY "Allow authenticated users to view scan test catalog"
    ON scan_test_catalog FOR SELECT
    TO authenticated
    USING (TRUE);

-- Allow authenticated users to view scan orders
CREATE POLICY "Allow authenticated users to view scan orders"
    ON scan_test_orders FOR SELECT
    TO authenticated
    USING (TRUE);

-- Allow authenticated users to create scan orders
CREATE POLICY "Allow authenticated users to create scan orders"
    ON scan_test_orders FOR INSERT
    TO authenticated
    WITH CHECK (TRUE);

-- Allow staff to update scan orders
CREATE POLICY "Allow authenticated users to update scan orders"
