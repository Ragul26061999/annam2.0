-- Fix stock_transactions constraint violation on negative deltas
-- stock_transactions.check_transaction_quantity allows negative quantity only for certain transaction_type values
-- Use purchase_return for negative deltas produced by purchase item deletion/reduction
...
BEGIN
  ...
  IF v_delta_qty <> 0 THEN
    ...
    v_txn_type := CASE WHEN v_delta_qty > 0 THEN 'purchase' ELSE 'purchase_return' END;
