-- Update RPC function to include pack size and pack price information
-- This adds pack_size, pack_mrp, and unit_purchase_price to batch data
...
BEGIN
    ...
    FROM (
        SELECT jsonb_build_object(
            ...
            END,
            'pack_size', dpi.pack_size,
            'pack_mrp', mb.selling_price,
            'pack_purchase_price', mb.purchase_price,
            'unit_purchase_price', CASE 
                WHEN dpi.pack_size > 0 AND dpi.pack_size IS NOT NULL 
                THEN ROUND(mb.purchase_price / dpi.pack_size, 2)
                ELSE mb.purchase_price
            END,
            'status', mb.status,
